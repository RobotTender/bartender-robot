...
- **비전 기반 실시간 부피 추정**: YOLO26 Segmentation + 비율 기반 룩업 테이블 + EMA 평활화로 정량 제어
...
| **Detection (비전)** | 병 위치 탐지 + 액체 부피 측정 | YOLO26, RealSense D455F, OpenCV |
...
bartender-robot/
├── detection/
│   ├── realsense_cam1.py        # 카메라 1: 병 위치 탐지 (YOLO26 + 깊이)
│   ├── realsense_cam2.py        # 카메라 2: 액체 부피 측정 (YOLO26 Seg + EMA)
│   └── weights/
│       ├── cam_1.pt             # 병 탐지 모델
│       └── cam_2.pt             # 액체 분할 모델
├── llm/
...
- **모델**: YOLO26 (`weights/cam_1.pt`), confidence ≥ 0.5
...
- **비전-로봇 좌표 변환**: RGB + Depth 프레임 동기화 후 YOLO26로 병 탐지, 사전 보정된 회전 행렬(R)과 이동 벡터(t)로 픽셀 → 로봇 베이스 좌표 변환
