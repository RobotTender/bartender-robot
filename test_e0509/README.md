...
- arm 제어 제한은 기본적으로 `30 deg/s`, `100 deg/s^2`입니다 (`arm_joint_speed_limit_deg_s`, `arm_joint_acc_limit_deg_s2`).
...
`jointwise tolerance` 의미:

- 단순히 "전체 조인트 오차 norm이 작다"만 보는 방식이 아니라, 각 조인트(`J1~J6`) 오차가 각각 허용범위 안에 들어와야 성공으로 판정하는 방식입니다.
- 그래서 특정 조인트 하나가 크게 틀어졌는데 다른 조인트가 맞아서 전체 norm만 작게 나오는 경우를 걸러낼 수 있습니다.
- 즉, "대충 비슷한 자세"보다 "각 축까지 실제 목표 자세에 가깝게 맞는지"를 더 엄격하게 검사하는 역할입니다.
- 현재 MoveBottle에서는 Stage1/Stage2/Stage3로 갈수록 이 허용범위를 더 엄격하게 써서, 마지막 단계일수록 자세 정합도를 더 강하게 요구합니다.
...
1. `0:6` arm joint command (normalized `[-1, 1]`)

설명:

- 정책 raw 출력값 자체는 단위가 없는 연속 실수값이며, 각 원소가 보통 `[-1, 1]` 범위에서 나옵니다.
- 각 액션 값은 해당 조인트의 한 step 증분 명령으로 사용됩니다.
- 물리량으로 해석된 뒤의 실질 단위는 joint target increment `[rad/step]`입니다.
- 따라서 실전에서 출력값을 보정할 때는 raw `[-1,1]` 값만 보지 말고, 최종 증분량(`[rad/step]` 또는 환산된 `[deg/step]`) 기준으로 생각하는 것이 안전합니다.
- 내부적으로는 `target += speed_scale * dt * action * action_scale * pre_grasp_arm_scale` 형태로 누적 적용됩니다.
- 즉, `-1`/`+1`은 해당 step에서 허용된 최대 음/양 방향 증분 명령에 해당합니다. 예를 들어 `action[2] = 0.5`면 joint 3에 최대 양의 증분의 절반을 주는 의미입니다.
- 현재 등록된 GripBottle은 `action_space = 6`이므로 그리퍼 개폐 액션은 없습니다.
- 코드상 `action[6]` 그리퍼 경로는 남아 있지만, 이는 액션 차원을 7 이상으로 확장했을 때만 사용됩니다.
...
1. `0:6` arm desired velocity command (normalized `[-1, 1]`)

설명:

- 정책 raw 출력값 자체는 단위가 없는 연속 실수값이며, 각 원소가 보통 `[-1, 1]` 범위에서 나옵니다.
- 각 액션 값은 해당 조인트의 목표 속도 비율로 해석됩니다.
- 물리량으로 해석된 뒤의 1차 단위는 desired joint velocity `[rad/s]`입니다.
- 따라서 실전에서 출력값을 보정할 때는 raw `[-1,1]` 값만 보지 말고, 최종 속도(`[rad/s]` 또는 환산된 `[deg/s]`) 기준으로 생각하는 것이 안전합니다.
- 내부적으로는 먼저 `desired_vel = joint_speed_limit * action`으로 바꾼 뒤, 조인트 가속도 제한과 startup hold/ramp를 적용합니다.
- 그 다음 `target += cmd_vel * dt`로 적분되어 최종 joint target `[rad]`이 만들어집니다.
- 즉, `-1`/`+1`은 해당 조인트의 최대 음/양 방향 목표 속도 명령에 해당합니다. 현재 설정 기준으로 `action = 1.0`이면 최대 약 `+30 deg/s`, `action = 0.5`면 약 `+15 deg/s` 목표에 해당합니다.
- 현재 MoveBottle은 그리퍼를 시작부터 닫힘 상태로 고정해서 사용하므로, 정책 액션으로 그리퍼를 제어하지 않습니다.
...
- 관측 차원은 동일하지 않습니다.
  - Grip: `20`
  - Move: `22`
- `Isaac-E0509-Move-Bottle-Direct-v0`는 별도 "기본 Move 모드"가 아니라, 현재 등록상 `MoveBottleStage2EnvCfg`를 가리키는 기본 alias입니다.
