...
class ActionNode(Node):
    def __init__(self):
        super().__init__('robotender_action', namespace='dsr01')
        ...
        # Subscriptions
        self.create_subscription(Empty, 'pouring_trigger', self.trigger_cb, 10, callback_group=self.callback_group)
        ...
        # Services
        self.pour_srv = self.create_service(Trigger, 'pour/start', self.pour_callback, callback_group=self.callback_group)
        self.warmup_srv = self.create_service(Trigger, 'warmup/start', self.warmup_callback, callback_group=self.callback_group)
        
        # Timer for path recording (3Hz)
        self.record_timer = self.create_timer(0.33, self.record_loop, callback_group=self.callback_group)

        self.get_logger().info('--- Robotender Action Node Initialized ---')
        self.get_logger().info('Services: /dsr01/pour/start, /dsr01/warmup/start')
